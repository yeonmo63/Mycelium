...
const MobileSettings = () => {
    ...
    const loadData = async () => {
        ...
        try {
            const [ips, mobileConfig] = await Promise.all([
                ...
                invoke('get_mobile_config').catch(() => ({ access_pin: '', use_pin: false }))
            ]);

            setAllIps(ips);

            // Filter usable IPs: No loopback, no APIPA
            const usableIps = ips.filter(ip => ip !== '127.0.0.1' && !ip.startsWith('169.254.'));

            // Prioritize: 192.168.x.x > 100.x.x.x (Tailscale) > others
            const initialIp = usableIps.find(ip => ip.startsWith('192.168.'))
                || usableIps.find(ip => ip.startsWith('100.'))
                || usableIps[0]
                || '127.0.0.1';

            setSelectedIp(initialIp);
            setConfig(mobileConfig || { access_pin: '', use_pin: false });
        } catch (e) {
    ...
    const activeUrl = buildUrl(selectedIp);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
                    ...
                    <div className="lg:col-span-7 bg-white p-1 rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 border border-slate-100 flex flex-col relative overflow-hidden group">
                        <div className="p-10 flex flex-col items-center justify-center text-center relative z-10">
                            {/* View Mode Switch - Removed as requested */}
                            <div className="flex flex-col items-center justify-center mb-10">
                                <div className="px-6 py-2 bg-indigo-50 text-indigo-600 rounded-2xl text-[10px] font-black border border-indigo-100 flex items-center gap-2">
                                    <Smartphone size={14} /> 모바일 다이렉트 연결
                                </div>
                            </div>
                                    ...
                                    <div className="w-[300px] h-[300px] bg-slate-50 rounded-[2.5rem] flex flex-col items-center justify-center text-slate-300 gap-4 border-2 border-dashed border-slate-200">
                                        ...
                                        <div className="px-12 text-center">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 leading-tight block mt-2">
                                                유효한 네트워크 접속 주소를 생성할 수 없습니다. 우측 설정을 확인해 주세요.
                                            </span>
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-slate-400 font-bold max-w-sm mx-auto leading-relaxed">
                                    모바일 기기에서 이 코드를 스캔하여 농장 제어 대시보드에 즉시 접속합니다. (Wi-Fi 또는 Tailscale IP)
                                </p>
                            </div>
                        ...
                        <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100 relative overflow-hidden">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-slate-400 font-bold">접속 IP 주소</span>
                                    <select
                                        value={selectedIp}
                                    ...
                                    >
                                        {allIps.filter(ip => ip !== '127.0.0.1').length > 0 ? (
                                            allIps.filter(ip => ip !== '127.0.0.1').map(ip => (
                                                <option key={ip} value={ip}>
                                                    {ip} {ip.startsWith('100.') ? '(Tailscale)' : (ip.startsWith('169.254.') ? '(비활성)' : '(내부망)')}
                                                </option>
                                            ))
                                        ) : (
                            ...
                            <div className="flex-1 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                                    {[
                                        ...
                                        { step: "04", title: "연동 확인", desc: "상단 '접속 IP 주소'에서 Tailscale IP를 선택하세요.", icon: null }
                                    ].map((item, id) => (
