...
const SalesRowsTable = ({ salesRows, editingTempId, handleEditRow, handleDeleteRow }) => {
    return (
        <div className="flex-1 overflow-hidden flex flex-col bg-white rounded-[1.5rem] shadow-xl border border-slate-200 relative mb-2">
                ...
                <table className="w-full text-xs border-separate border-spacing-0">
                    <thead className="sticky top-0 z-20">
                        <tr className="bg-slate-50/80 backdrop-blur-md">
                            <th className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">번호</th>
                            <th className="px-4 py-1.5 text-left text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">상품명</th>
                            <th className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">규격</th>
                            <th className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">수량</th>
                            <th className="px-4 py-1.5 text-right text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">단가</th>
                            <th className="px-4 py-1.5 text-right text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">할인</th>
                            <th className="px-4 py-1.5 text-right text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">금액</th>
                            <th className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">상태</th>
                            <th className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">관리</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {salesRows.map((row, idx) => (
                            <tr key={row.tempId} className={`group hover:bg-slate-50/50 transition-all ${editingTempId === row.tempId ? 'bg-indigo-50/40' : ''}`}>
                                <td className="px-4 py-1.5 text-center text-[9px] font-black uppercase text-slate-400 border-b border-slate-100">{salesRows.length - idx}</td>
                                <td className="px-4 py-1.5 border-b border-slate-100">
                                ...
                                <td className="px-4 py-1.5 text-center border-b border-slate-100">
                                ...
                                <td className="px-4 py-1.5 text-center font-black text-black border-b border-slate-100">
                                    {row.qty}
                                </td>
                                <td className="px-4 py-1.5 text-right font-bold text-slate-500 border-b border-slate-100">{formatCurrency(row.price)}</td>
                                <td className="px-4 py-1.5 text-right font-bold text-indigo-500 border-b border-slate-100">{row.discountRate}%</td>
                                <td className="px-4 py-1.5 text-right border-b border-slate-100">
                        ...
                        {salesRows.length === 0 && (
                            <tr>
                                <td colSpan="9" className="pt-12 pb-6 text-center bg-white">
