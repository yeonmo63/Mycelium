    ...
    const handleConvert = async () => {
        ...
        try {
            if (window.__TAURI__) {
                ...
                await showAlert("완료", "생산 처리가 완료되었습니다.");
                ...
                <div className="flex justify-between items-end mb-4">
                    <div>
                        ...
                        <h1 className="text-3xl font-black text-slate-600 tracking-tighter" style={{ fontFamily: '"Noto Sans KR", sans-serif' }}>
                            재고 관리 <span className="text-slate-300 font-light ml-1 text-xl">Stock Control</span>
                        </h1>
                    </div>
                    {tab === 'product' && (
                        <button onClick={openConvertModal} className="h-[42px] px-5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm shadow-lg shadow-indigo-200 flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-95">
                            <span className="material-symbols-rounded">potted_plant</span> 완제품 생산
                        </button>
                    )}
                </div>
            </div>
                    ...
                    <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white z-10">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-xl">
                            <button onClick={() => setTab('product')} className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${tab === 'product' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                                <span className="material-symbols-rounded text-base">potted_plant</span> 판매 상품
                            </button>
                            <button onClick={() => setTab('material')} className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${tab === 'material' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                                <span className="material-symbols-rounded text-base">package_2</span> 자재 창고
                            </button>
                        </div>
                        ...
                        <table className="w-full text-xs text-left border-collapse table-fixed">
                            <thead className="sticky top-0 bg-slate-50/95 backdrop-blur z-10 shadow-sm text-slate-500 uppercase font-bold tracking-wider">
                                <tr>
                                    ...
                                    <th className="px-2 py-3 w-[20%] border-b border-slate-100">상품명</th>
                    ...
                    <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                            <label className="flex items-center gap-1.5 cursor-pointer group">
                                ...
                                <span className="text-[10px] font-bold text-slate-400 group-hover:text-slate-600 transition-colors">자동 로그 숨김</span>
                            </label>
                                        ...
                                        <div key={idx} className="p-3 hover:bg-slate-50 transition-colors flex gap-3 items-start">
                                            <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${log.change_type === '입고' ? 'bg-blue-50 text-blue-600' :
                                                ...
                                                <span className="material-symbols-rounded text-lg">
                                                    {log.change_type === '입고' ? 'login' : log.change_type === '출고' ? 'logout' : 'sync_alt'}
                                                </span>
                                            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-[400px] shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white relative overflow-hidden">
                            <span className="material-symbols-rounded absolute -right-6 -top-6 text-[120px] text-white/10 pointer-events-none">potted_plant</span>
                            <h3 className="text-lg font-black flex items-center gap-2 relative z-10">
                                <span className="material-symbols-rounded">potted_plant</span> 완제품 생산 (전환)
                            </h3>
                            <p className="text-xs text-white/80 mt-1 relative z-10 font-medium">자재(박스 등)가 차감되고 완제품 재고가 늘어납니다.</p>
                        </div>

                        <div className="p-6">
                            <div className="mb-5">
                                <label className="text-xs font-bold text-slate-500 block mb-1.5 ml-1">생산 품목 선택</label>
                                ...
                                return m ? (
                                    <div className="bg-slate-50 p-4 rounded-xl mb-5 border border-slate-200">
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-xs font-bold text-slate-500">필요 자재</span>
                                        ...
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs font-bold text-slate-500">자재 재고</span>
                            ...
                            <div className="mb-6">
                                <label className="text-xs font-bold text-slate-500 block mb-1.5 ml-1">생산 수량</label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        min="1"
                                    ...
                                    />
                                    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">EA</div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={handleConvert} className="flex-1 h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm shadow-md shadow-indigo-200 transition-all hover:scale-[1.02] active:scale-95">생산 완료</button>
                            </div>
                        </div>
                    </div>
                </div>
